...
class EarlyStopping:
    def __init__(self, patience:int=7, verbose:bool=False, delta:int=0, path:str='checkpoint.pt'):
        """
        EarlyStopping class giúp dừng sớm việc huấn luyện mô hình nếu validation loss không giảm.
        Arguments:
        patience (int): Số epoch phải chờ nếu validation loss không giảm trước khi dừng.
        verbose (bool): Nếu True, in ra thông tin về quá trình lưu mô hình.
        delta (float): Sự thay đổi tối thiểu để validation loss được coi là đã cải thiện.
        path (str): Đường dẫn để lưu checkpoint mô hình.
        """
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        path_save = os.path.join(self.path, "best.pth")
